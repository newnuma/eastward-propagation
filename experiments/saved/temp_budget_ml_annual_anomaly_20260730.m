%% Mixed-layer temperature budget and annual-mean anomaly maps (2026-07-30)
% Derives the mixed-layer temperature budget and saves one all-year figure
% for each of the eight budget and closure terms.

exp = init_experiment('20260730_temp_budget_ml_annual_anomaly');
cfg = exp.cfg;
grid = exp.grid;

%% User settings
% true: recalculate the budget; false: reuse temp_budget_ml.mat when present.
recompute_budget = false;
refresh_heat_flux = false;
...
% Defaults are rounded 98th-percentile limits within the plotted domain
% from the saved 2026-07-19 mixed-layer temperature budget.
plot_options.color_limits.default.tendency = [-0.15 0.15];
plot_options.color_limits.default.surface_forcing = [-1 1];
plot_options.color_limits.default.entrainment = [-0.4 0.4];
plot_options.color_limits.default.advection_zonal = [-0.07 0.07];
plot_options.color_limits.default.advection_meridional = [-0.25 0.25];
plot_options.color_limits.default.advection_total = [-0.25 0.25];
plot_options.color_limits.default.rhs_total = [-1 1];
plot_options.color_limits.default.residual = [-1 1];

% Optional mixed-layer override example:
% plot_options.color_limits.ml.tendency = [-0.25 0.25];

%% Derive or load the mixed-layer temperature budget
budget_relative_path = fullfile( ...
    cfg.paths.analysis, 'temp_budget_ml.mat');
...
stale_budget = budget_output_is_stale(cfg, budget_file, 'temp');
needs_budget_computation = recompute_budget || ...
    updated_dynamics.gvel || updated_dynamics.wind || ...
    refresh_heat_flux || stale_budget;

if needs_budget_computation
    flux_file = fullfile( ...
        cfg.paths.data_root, cfg.paths.base_data, 'flux.mat');
    if refresh_heat_flux || ~isfile(flux_file)
        read_ncep_flux(cfg);
    end
    compute_mld(cfg);
    temp_budget_ml = compute_temp_budget(cfg, "ml");
else
    fprintf('[experiment] Loading saved mixed-layer temperature budget.\n');
    temp_budget_ml = load_var( ...
        cfg, budget_relative_path, 'temp_budget_ml');
end

%% Save all-year annual-mean anomaly figures
plot_temp_budget_annual_anomaly_terms( ...
    temp_budget_ml, grid, 'ml', 'Mixed layer', 'temp_budget_ml', ...
...
settings.refresh_heat_flux = refresh_heat_flux;
settings.refresh_dynamics = refresh_dynamics;
exp.cfg = cfg;
save_experiment(exp, 'settings', settings);

fprintf('[experiment] Mixed-layer temperature budget workflow complete.\n');

function plot_temp_budget_annual_anomaly_terms( ...
        budget, grid, layer_key, layer_label, file_prefix, output_dir, opts)
%PLOT_TEMP_BUDGET_ANNUAL_ANOMALY_TERMS Save one all-year figure per term.
    ...
    for i = 1:numel(required_options)
        if ~isfield(opts, required_options{i})
            error('experiment:MissingPlotOption', ...
                'Missing temperature-budget plot option: %s', ...
                required_options{i});
        end
    end
...
%MAKE_TERM_MAP_GRID Plot all available years for one temperature budget term.
    ...
    color_bar.Label.String = 'Annual-mean temperature-change anomaly';
