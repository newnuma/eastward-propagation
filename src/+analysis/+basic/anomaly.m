...
%ANALYSIS.BASIC.ANOMALY Compute climatology, anomaly, yearly mean.
%
%   data = analysis.basic.anomaly(data, grid)
%
%   Input:
%       data : struct with .raw field (lon x lat x time)
%       grid : struct with .lon, .lat, .time, .year from io.load_grid
%
%   Adds fields:
