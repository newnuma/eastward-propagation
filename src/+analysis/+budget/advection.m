...
%ANALYSIS.BUDGET.ADVECTION Compute horizontal heat advection term.
%
%   result = analysis.budget.advection(cfg, grid, alldata, mld, depth_mode)
%
...
%       result.y.raw : meridional advection term
    ...
    wind = io.load_var(cfg, fullfile(cfg.paths.base_data, 'wind.mat'), 'wind');
    gvel = io.load_var(cfg, fullfile(cfg.paths.base_data, 'gvel.mat'), 'gvel');
    ...
    % --- Depth average ---
    if ischar(depth_mode) || isstring(depth_mode)
        [Dtx_mean, ~] = analysis.basic.mld_mean(Dtx, grid, pden, mld);
        [Dty_mean, ~] = analysis.basic.mld_mean(Dty, grid, pden, mld);
    else
        Dtx_mean = analysis.basic.depth_mean(Dtx, pres, 1:max_k);
        Dty_mean = analysis.basic.depth_mean(Dty, pres, 1:max_k);
    end

    result.x.raw = Dtx_mean;
    result.x = analysis.basic.anomaly(result.x, grid);

    result.y.raw = Dty_mean;
    result.y = analysis.basic.anomaly(result.y, grid);
end
