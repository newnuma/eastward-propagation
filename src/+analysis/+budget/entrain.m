...
%ANALYSIS.BUDGET.ENTRAIN Compute entrainment term in mixed-layer budget.
%
%   result = analysis.budget.entrain(cfg, grid, alldata, mld, depth_mode)
%
%   -(T_mean - T_b) / h * w_e
...
%       result.raw : entrainment term (lon x lat x time)
    ...
    wh_out = analysis.budget.wh(cfg, grid, mld, depth_mode);

    if ischar(depth_mode) || isstring(depth_mode)
        depth = mld.depth;
        [mean_data, bottom_now] = analysis.basic.mld_mean(alldata, grid, pden, mld);
        ...
        mean_data = analysis.basic.depth_mean(alldata, pres, 1:idx);
