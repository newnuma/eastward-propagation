...
%ANALYSIS.BUDGET.SALT_FLUX Surface salt flux from evaporation - precipitation.
%
%   result = analysis.budget.salt_flux(cfg, grid, mld, depth_mode)
%
%   (E - P) * S / (rho * h) * dt
%
%   Inputs:
...
%       result.raw : salt flux term (lon x lat x time)
    ...
    evp_pre = io.load_var(cfg, fullfile(cfg.paths.analysis, 'evp_pre.mat'), 'evp_pre');
