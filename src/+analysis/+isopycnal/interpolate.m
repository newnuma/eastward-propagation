...
%ANALYSIS.ISOPYCNAL.INTERPOLATE Interpolate temperature onto isopycnal surfaces.
%
%   results = analysis.isopycnal.interpolate(pden, temp, grid, target_densities)
%
%   Inputs:
...
%           .nan_count    — NaN count per month (lon x lat x 12)
    ...
    for di = 1:numel(target_densities)
        ...
        results.(field).depth = analysis.basic.anomaly(results.(field).depth, grid);
        results.(field).depth = analysis.basic.anomaly_detrend(results.(field).depth, grid);
        results.(field).temp  = analysis.basic.anomaly(results.(field).temp, grid);
        results.(field).temp  = analysis.basic.anomaly_detrend(results.(field).temp, grid);
    end
end
