...
%INGEST.READ_MOAA_DENSITY Read MOAA GPV potential density & dynamic height.
%
...
%       base_data/dheight.mat — dynamic height     (lon x lat x depth x time)
    ...
    io.save_var(cfg, fullfile(base, 'pden.mat'),    'pden',    all_pden);
    io.save_var(cfg, fullfile(base, 'dheight.mat'), 'dheight', all_dheight);

    io.update_manifest(cfg, 'base_data', 'pden',    {});
    io.update_manifest(cfg, 'base_data', 'dheight', {});
