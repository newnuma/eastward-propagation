...
%INGEST.READ_MOAA_TEMP_SAL Read MOAA GPV temperature & salinity NetCDF files.
%
...
%       base_data/grid.mat  — grid coordinates (lon, lat, pres, time, year)
    ...
    io.save_var(cfg, fullfile(base, 'grid.mat'), 'grid', grid);
    io.save_var(cfg, fullfile(base, 'temp.mat'), 'temp', all_temp);
    io.save_var(cfg, fullfile(base, 'sal.mat'),  'sal',  all_sal);

    io.update_manifest(cfg, 'base_data', 'grid', {});
    io.update_manifest(cfg, 'base_data', 'temp', {});
    io.update_manifest(cfg, 'base_data', 'sal',  {});
