...
%INGEST.READ_NCEP_EVP_PRE Read NCEP evaporation/precipitation data and regrid.
%
...
%   with fields: .prate (precipitation rate), .skt (skin temperature)
    ...
    % Regrid to target
    evp_pre.prate = ingest.regrid_to_target(prate_sub, sub_lon, sub_lat, grid.lon, grid.lat);
    evp_pre.skt   = ingest.regrid_to_target(skt_sub,   sub_lon, sub_lat, grid.lon, grid.lat);

    io.save_var(cfg, fullfile(cfg.paths.base_data, 'evp_pre.mat'), 'evp_pre', evp_pre);
    io.update_manifest(cfg, 'base_data', 'evp_pre', {});
