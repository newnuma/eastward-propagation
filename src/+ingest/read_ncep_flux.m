...
%INGEST.READ_NCEP_FLUX Read NCEP heat flux data and regrid to target grid.
%
...
%   and saves base_data/flux.mat with fields: .net, .lh, .sh, .lw, .sw
    ...
    flux.net = ingest.regrid_to_target(net, sub_lon, sub_lat, grid.lon, grid.lat);
    flux.lh  = ingest.regrid_to_target(lh,  sub_lon, sub_lat, grid.lon, grid.lat);
    flux.sh  = ingest.regrid_to_target(sh,  sub_lon, sub_lat, grid.lon, grid.lat);
    flux.lw  = ingest.regrid_to_target(lw,  sub_lon, sub_lat, grid.lon, grid.lat);
    flux.sw  = ingest.regrid_to_target(sw,  sub_lon, sub_lat, grid.lon, grid.lat);

    io.save_var(cfg, fullfile(cfg.paths.base_data, 'flux.mat'), 'flux', flux);
    io.update_manifest(cfg, 'base_data', 'flux', {});
