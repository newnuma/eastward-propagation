...
%INGEST.READ_NCEP_SLP Read NCEP sea level pressure and regrid to target grid.
%
...
%   Saves base_data/slp.mat with field .raw (lon x lat x time)
    ...
    slp.raw = ingest.regrid_to_target(slp_sub, sub_lon, sub_lat, grid.lon, grid.lat);

    io.save_var(cfg, fullfile(cfg.paths.base_data, 'slp.mat'), 'slp', slp);
    io.update_manifest(cfg, 'base_data', 'slp', {});
