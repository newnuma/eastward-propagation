...
%INGEST.READ_NCEP_WIND Read NCEP wind stress, compute curl, regrid.
%
...
%   saves base_data/wind.mat with fields: .taux, .tauy, .curl
    ...
    % Compute wind stress curl on source grid (dynamics.wsc expects nlat x nlon)
    uf_t = permute(uf, [2 1 3]);    % (nlat x nlon x nt)
    vf_t = permute(vf, [2 1 3]);
    curl_src = NaN(size(uf));       % (nlon x nlat x nt)
    for t = 1:nt
        curl_src(:, :, t) = dynamics.wsc(sub_lat, sub_lon, uf_t(:,:,t), vf_t(:,:,t));
    end

    % Regrid to target
    wind.taux = ingest.regrid_to_target(uf,       sub_lon, sub_lat, grid.lon, grid.lat);
    wind.tauy = ingest.regrid_to_target(vf,       sub_lon, sub_lat, grid.lon, grid.lat);
    wind.curl = ingest.regrid_to_target(curl_src, sub_lon, sub_lat, grid.lon, grid.lat);

    io.save_var(cfg, fullfile(cfg.paths.base_data, 'wind.mat'), 'wind', wind);
    io.update_manifest(cfg, 'base_data', 'wind', {});
