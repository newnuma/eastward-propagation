...
for t=1:TIM
    for k=1:h
        for j=1:91
            for i=2:140
                dT_dx(i,j,k,t)=(salltemp(i+1,j,k,t)-salltemp(i-1,j,k,t))/(cos(slat(j)/180*pi)*(2/180)*pi*R);
            end
        end
    end
end
...
for t=1:TIM
    for k=1:h
        for j=2:90
            for i=2:140
                dT_dy(i,j,k,t)=(salltemp(i,j+1,k,t)-salltemp(i,j-1,k,t))/((2/360)*2*pi*R);
            end
        end
    end
end
...
% エクマン輸送
Ue=wind_moment.y./mldD;
Ue=Ue./rho_f;

Ve=-wind_moment.x./mldD;
Ve=Ve./rho_f;
...
mlhb.adx.v = Dtx_mld;
mlhb.adx = anomaly(mlhb.adx);

mlhb.ady.v = Dty_mld;
mlhb.ady = anomaly(mlhb.ady);
% 
% 
% 
% % allsal1=permute(Dtx,[4 1 2 3]);      
% % allsal2=reshape(allsal1,[TIM 141*91*9]);     
% % clear allsal1
% % allsal3=array2timetable(allsal2,'RowTimes',time);   %timetableに変換
% % clear allsal2
% % %
% % mm1=groupsummary(allsal3,'Time','monthofyear','mean');   %月別の気候値、tableカテゴリカル
% % clear allsal3
% % mm2=table2array(mm1(:,3:end));%行列に変換、カテゴリカル部分除く
% % clear mm1
% % mm3=reshape(mm2,[12 141 91 9]);        %3次元に戻す
% % clear mm2
% % mc=permute(mm3,[2 3 4 1]);              %気候値(月12×経度360×緯度155）
% % mc1=repmat(mc,[1 1 1 20]);   %気候値をデータの年数分。データと同じサイズ
% % Dtxa=Dtx-mc1; %%
% % 
% % allsal1=permute(Dty,[4 1 2 3]);      
% % allsal2=reshape(allsal1,[TIM 141*91*9]);     
% % clear allsal1
% % allsal3=array2timetable(allsal2,'RowTimes',time);   %timetableに変換
% % clear allsal2
