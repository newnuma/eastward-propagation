...
%       residual        = tendency - rhs_total
    ...
    % Direct calls must not silently use dynamics generated by an older
    % version of the velocity, stress-sign, or curl algorithms.
    ...
    budget.algorithm_version = '2.0';
    budget.generated_at = datetime('now', 'TimeZone', 'UTC');
    ...
    elseif depth_index == 1
        ...
        layer_mean = fixed_layer_mean(tracer, pres, depth_index);
    end
    budget.tendency = temporal_tendency(layer_mean, grid);
    fprintf('  tendency done\n');

    budget = finalize_tracer_budget( ...
        budget, grid, cfg.analysis.budget_min_annual_months);
