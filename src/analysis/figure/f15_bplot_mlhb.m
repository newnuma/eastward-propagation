savename='f15_bplot_mlhb.png';

blon=92:111; blat=61:70; %選択領域
start_time=12*10+1;%表示開始月
...
x1=squeeze(mean(mlhb.dt.a(blon,blat,:),[1 2],'omitnan'));
x1=movmean(x1,moveM,1);
x1_title='change rate of temperature';

x2=squeeze(mean(mlhb.asf.a(blon,blat,:),[1 2],'omitnan'));
x2=movmean(x2,moveM,1);
x2_title='surface heat flux';

x3=squeeze(mean(mlhb.adx.a(blon,blat,:),[1 2],'omitnan'));
x3=movmean(x3,moveM,1);
x3_title='zonal heat advection';

x4=squeeze(mean(mlhb.adx.a(blon,blat,:),[1 2],'omitnan'));
x4=movmean(x4,moveM,1);
x4_title='meridional heat advection';
...
ena_wh=squeeze(mean(mlhb.entrain.a(blon,blat,:),[1 2],'omitnan'));
%%8割以上のデータがあるとき
ent_wh=mlhb.entrain.v(blon,blat,:);
for t=1:numel(time)
...
legend(x1_title,x2_title,x3_title,x4_title,"entrainment", 'Location', 'northeast')%,'エントレインメント'
ylim(value_lange);
