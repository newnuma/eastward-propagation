...
dtxaB=squeeze(mean(dtxa(blon,blat,bt),[1 2],'omitnan'));
dtyaB=squeeze(mean(dtya(blon,blat,bt),[1 2],'omitnan'));
sf=squeeze(mean(asfa(blon,blat,bt),[1 2]));
tmd=squeeze(mean(Tmda(blon,blat,bt),[1 2]));
mldtm=squeeze(mean(-ssfa(blon,blat,bt)./80,[1 2]));
%%8割以上のデータがあるとき
...
ylim([-0.5 0.5])
legend('水温変化','海面熱フラックス(温度変化)','海面熱フラックス/80')
%legend('移流ｘ','移流ｙ')
hold off