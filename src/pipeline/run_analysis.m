...
%     6. Basic field processing (depth means, anomalies)
    ...
    iso_results = analysis.isopycnal.interpolate( ...
        pden, temp, grid, cfg.analysis.target_isopycnals);

    io.save_var(cfg, fullfile(cfg.paths.analysis, 'isopycnal.mat'), ...
        'iso_results', iso_results);
    io.update_manifest(cfg, 'analysis', 'isopycnal', {'pden', 'temp'});
    ...
    sb150 = analysis.budget.cal_fixdepth_sb(cfg, 8);  % index 8 = 150m
...
%COMPUTE_BASIC_FIELDS Compute depth-averaged anomaly fields for key variables.
    ...
    temp_z10 = analysis.basic.anomaly(temp_z10, grid);
    temp_z10 = analysis.basic.anomaly_detrend(temp_z10, grid);

    % 10-150m average
    temp_z10_150.raw = analysis.basic.depth_mean(temp, pres, 1:8);
    temp_z10_150 = analysis.basic.anomaly(temp_z10_150, grid);
    temp_z10_150 = analysis.basic.anomaly_detrend(temp_z10_150, grid);

    sal_z10_150.raw = analysis.basic.depth_mean(sal, pres, 1:8);
    sal_z10_150 = analysis.basic.anomaly(sal_z10_150, grid);
    sal_z10_150 = analysis.basic.anomaly_detrend(sal_z10_150, grid);

    pden_z10_150.raw = analysis.basic.depth_mean(pden, pres, 1:8);
    pden_z10_150 = analysis.basic.anomaly(pden_z10_150, grid);
    pden_z10_150 = analysis.basic.anomaly_detrend(pden_z10_150, grid);
    ...
    io.save_var(cfg, fullfile(cfg.paths.analysis, 'fields.mat'), 'fields', fields);
    io.update_manifest(cfg, 'analysis', 'fields', {'temp', 'sal', 'pden'});
